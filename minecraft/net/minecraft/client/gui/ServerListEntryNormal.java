...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry
{
    ...
    private static final int serverCountCache = new ServerList(Minecraft.getMinecraft()).countServers();
    ...
    public Future<?> pingServer() {
        ...
        return field_148302_b.submit(new Runnable() {
            public void run() {
                try {
                    future.get(SERVER_TIME_OUT, TimeUnit.SECONDS);
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected)
    {
        if (!this.server.field_78841_f)
        {
            ...
            //TODO watch, new server pinger
            field_148302_b.submit((Runnable) pingServer());
